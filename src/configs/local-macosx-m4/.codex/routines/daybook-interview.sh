...
mkdir -p "$HOME/.codex/routines/logs"
...
#
# dynamic_title=false pins the window title to "Daybook" so the running Codex
...
exec "$ALACRITTY" --title "Daybook" -o window.dynamic_title=false -e "$HOME/.codex/routines/daybook-interview-session.command"
