...
def setup_plotting(style: str = "fivethirtyeight", figsize: tuple = (12, 6)):
    """Configure matplotlib/seaborn with sensible defaults.

    Uses the fivethirtyeight style by default for clean, publication-ready plots.
    Falls back to seaborn whitegrid if fivethirtyeight is unavailable.
    """
    import matplotlib.pyplot as plt
    import seaborn as sns

    # Try to use fivethirtyeight style
    try:
        plt.style.use(style)
    except OSError:
