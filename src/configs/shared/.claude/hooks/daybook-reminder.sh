...
echo '{"decision":"block","reason":"Before stopping, check the daybook. Search Joplin for today'\''s note in Areas / Daybook (title format: DD Mon, YYYY). If it exists, read it and append any work from this session that is not already covered. If it does not exist, create it. Entry format: - HH:MM — <one-sentence summary>. The current time is '"$NOW"'. Use that for the timestamp — do NOT call date separately. Include a screenshot link if visual changes were made. If this session was actually trivial (casual chat, no code/config changes), you may stop without logging. When creating a new Daybook entry for today, look at the most recent Daybook entry and if there is a "To Do" or similar checklist, copy over the incomplete items to a To Do checklist in the new note. This helps maintain a running list of tasks across days. Below the To Do checklist, there should be a section with header "Worklog"}'
exit 0
