...
get_xid_from_key() {
	...
	cache_p | grep "^$WINDOW_KEY " | awk '{print $3}' | tail -1	#TODO: handle multiples?
}
...
get_pid_from_key() {
	...
	cache_p | grep "^$WINDOW_KEY " | awk '{print $2}' | tail -1	#TODO: handle multiples?
}
...
key_exists() {
	...
	RESULT="false"
	if [ ! -z $(cache_p | grep "^$WINDOW_KEY " | awk '{print $2}') ]; then
		RESULT="true"
	fi
	echo $RESULT
}
...
cache_d() {
	...
	echo "$(cache_p | grep -v "^$WINDOW_KEY ")" > $_CACHE
}
...
kill_one() {
	...
	kill -9 $(cache_p | grep "^$WINDOW_KEY " | tail -1 \
		| awk '{print $2}')
	cache_d $WINDOW_KEY
}
...
kill_set() {
	...
	WINDOW_KEYS=($(cache_p | grep " $SET_KEY" | awk '{print $1}'))
